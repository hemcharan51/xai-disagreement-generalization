...
|---|---|---|
| **RA** (Rank Agreement) | Spearman correlation of importance ranks over all features | = paper rank correlation over `F = all` |
| **SA** (Sign Agreement) | fraction of *all* features with matching attribution sign | over the full feature set, not only top-k |
| **SRA** (Signed Rank Agreement, **primary**) | `(1/|F|) Σ_f 1[S_a(f)=S_b(f)] · (1 − |R_a(f)−R_b(f)|/|F|)` | see note below |
...
implement the **signed weighted** form above — the paper's own *weighted rank
agreement* idea (Appendix D.2) fused with the sign indicator. It is continuous,
equals 1 for identical explanations, and rewards both sign agreement and rank
closeness. The strict top-k version is still available as
...
per-instance mean pairwise SRA: a **positive** slope means degraded, uncertain
predictions show *spuriously high* agreement (flat gradient landscapes →
near-zero attributions → methods trivially agree on "nothing matters").
